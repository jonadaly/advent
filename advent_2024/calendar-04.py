...
def get_diagonals(char_array: np.ndarray) -> list[np.ndarray]:
    return [
        np.diagonal(char_array[i:, :], axis1=0, axis2=1)
        for i in reversed(range(1, char_array.shape[0]))
    ] + [
        np.diagonal(char_array[:, i:], axis1=0, axis2=1)
        for i in range(0, char_array.shape[0])
    ]


grid_diagonals = get_diagonals(grid)
grid_diagonals_other = get_diagonals(grid_rotated)

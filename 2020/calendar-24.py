...
def advance_day(tiles: Dict[Tuple[float, float], bool]) -> Dict[Tuple[float, float], bool]:
    ...
    for coords, flipped in tiles.items():
        ...
        for surrounding in [
            ...
            new_coords = (round(coords[0] + surrounding[0], 5), round(coords[1] + surrounding[1], 5))
            if round_for_comparison(new_coords) not in [round_for_comparison(t) for t in expanded_tiles.keys()]:

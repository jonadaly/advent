...
inputs = """[1518-05-27 00:42] falls asleep
...
# inputs = """[1518-11-01 00:00] Guard #10 begins shift
...
for entry in entries:
    result = re.search(r"\[\d\d\d\d-\d\d-\d\d \d\d:(\d\d)\]\s(.*)", entry)
            ...
            shifts.append(
                {
                    "date": "somedate",
                    "guard_id": guard_id,
                    "falls_mins": falls_mins,
                    "wakes_mins": wakes_mins,
                }
            )
        guard_id = re.search(r"Guard #(\d+) begins shift", note).group(1)
        falls_mins = []
        wakes_mins = []
    elif note.startswith("falls"):
        falls_mins.append(minute)
    elif note.startswith("wakes"):
        wakes_mins.append(minute)
    else:
        raise ValueError
...
guards = {}
for shift in shifts:
    guards[shift["guard_id"]] = {"record": np.zeros(60)}

for shift in shifts:
    assert len(shift["falls_mins"]) == len(shift["wakes_mins"])
    for i in range(len(shift["falls_mins"])):
        guards[shift["guard_id"]]["record"][
            shift["falls_mins"][i] : shift["wakes_mins"][i]
        ] += 1


for guard in guards.values():
    guard["total_minutes"] = np.sum(guard["record"])

pprint.pprint(guards)

max_minutes = 0
for k, guard in guards.items():
    if guard["total_minutes"] > max_minutes:
        max_minutes = guard["total_minutes"]
        max_guard_id = k
        max_min_index = np.argmax(guard["record"])
        max_min_times = np.amax(guard["record"])

print(f"Part 1: Guard {max_guard_id}: minute {max_min_index} ({max_min_times} times)")


max_ind_minute = 0
for k, guard in guards.items():
    if np.amax(guard["record"]) > max_ind_minute:
        max_ind_minute = np.amax(guard["record"])
        max_ind_guard_id = k
        max_ind_min_index = np.argmax(guard["record"])

print(
    f"Part 2: Guard {max_ind_guard_id}: minute {max_ind_min_index} ({max_ind_minute} times)"
)

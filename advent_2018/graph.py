...
if not os.path.exists(
    "leader_board_" + LEADER_BOARD_CODE + ".json"
) or os.path.getmtime("leader_board_" + LEADER_BOARD_CODE + ".json") < (
    int(time.time()) - 60 * 30
):
...
for member_id, member_details in board["members"].items():
    for day, level_details in member_details["completion_day_level"].items():
        ...
        if "2" in level_details:
            history[level_details["2"]["get_star_ts"]] = (
                member_details["name"],
                day,
                "2",
            )
...
dayScores = {
    str(x): {str(z): [y for y in range(1, len(people) + 1)] for z in range(1, 3)}
    for x in range(1, 26)
}
scores = {p: {"x": [0], "star_y": [0], "points_y": [0]} for p in people}
for ts, event in history.items():
    event_date = datetime.fromtimestamp(int(ts))
    print(
        event_date.strftime("%Y-%b-%d %H:%M:%S")
        + " - "
        + event[0]
        + " completed Day "
        + event[1]
    ...
    scores[event[0]]["points_y"].append(
        scores[event[0]]["points_y"][-1]
        + (dayScores[event[1]][event[2]].pop() if event[1] != "6" else 0)
    )

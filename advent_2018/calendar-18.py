...
for i in range(1000000000):
    ...
    if n_woods_this in n_woods and n_lumbers_this in n_lumbers:
        if n_woods.index(n_woods_this) and n_lumbers.index(n_lumbers_this):
            ...
            pattern_end = i - 1
            print(
                f"Found pattern. {pattern_start}-{pattern_end}. Modulo {1 + pattern_end - pattern_start}"
            )
            ind = pattern_start + (
                (1000000000 - pattern_start) % (1 + pattern_end - pattern_start)
            )
            ...
            print(
                f"Part 2: {n_woods[ind]} woods and {n_lumbers[ind]} lumberyards, score: {n_woods[ind]*n_lumbers[ind]}"
            )
            break
    ...
    if i == 9:
        print(
            f"Part 1: {n_woods_this} woods and {n_lumbers_this} lumberyards, score: {n_woods_this*n_lumbers_this}"
        )

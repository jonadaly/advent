    ...
    def is_surrounded(self, fighters):
        ...
        for fighter in fighters:
            if fighter.hp > 0:
                temp_cav[fighter.y, fighter.x] = fighter.char
        return (
            temp_cav[self.y - 1, self.x] != b"."
            and temp_cav[self.y + 1, self.x] != b"."
            and temp_cav[self.y, self.x + 1] != b"."
            and temp_cav[self.y, self.x - 1] != b"."
        )
    ...
    while targets_exist is True and any_dead_elf is False:
        for fighter in sorted(fighters, key=lambda f: f.y * 9999999 + f.x):
            ...
            if target is not None:
                ...
                for open_space in sorted(
                    open_spaces, key=lambda s: s[1] * 99999 + s[0]
                ):
                    cand_path = Fighter.find_path(
                        (fighter.x, fighter.y), open_space, fighters
                    )
...
while True:
    ...
    if any_dead_elf is False:
        print(
            f"Part 2: {rounds} full rounds, {total_hp} total HP remaining. Score {total_hp*rounds}"
        )
        break
    elf_power += 1
# 167280

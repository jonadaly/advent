...
def get_possible_codes(sample1):
    ...
    if (
        A > sample1["before"][B]
        and sample1["after"][C] == 1
        or A <= sample1["before"][B]
        and sample1["after"][C] == 0
    ):
        possible_ops.append("gtir")
    if (
        sample1["before"][A] > B
        and sample1["after"][C] == 1
        or sample1["before"][A] <= B
        and sample1["after"][C] == 0
    ):
        possible_ops.append("gtri")
    if (
        sample1["before"][A] > sample1["before"][B]
        and sample1["after"][C] == 1
        or sample1["before"][A] <= sample1["before"][B]
        and sample1["after"][C] == 0
    ):

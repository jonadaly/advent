...
for i in range(100000):
	for c in sorted(carts, key=lambda c: c.y*1000 + c.x):
		...
		for d in carts:
			if c.x == d.x and c.y == d.y and c.char != d.char and c.crashed is False and d.crashed is False:
	...
	if len(uncrashed_carts) == 1:
		# print_track(track, carts)
		print(f"Only 1 uncrashed cart left at ({uncrashed_carts[0].x},{uncrashed_carts[0].y})")
		break
	elif len(uncrashed_carts) == 0:
		print(f"No uncrashed carts!!!")
		exit()
	# print_track(track, carts)

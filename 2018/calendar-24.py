...
class Group:
    ...
    def __init__(self, idx, team, units, hp, damage, damage_type, immunities, weaknesses, initiative):
        self.idx = idx
        self.team = team
        self.units = units
        self.hp = hp
...
def parse_group(line, team, idx, boost):
    ...
    return Group(idx, team, units, hp, damage, damage_type, immunities, weaknesses, initiative)
...
def select_targets(groups):
    ...
    for group in groups:
        ...
        best_total_damage = 0
        best_target = None
        for cand in [g for g in groups if g.team != group.team and g.targeted is False and g.units > 0]:
            ...
            if total_damage == best_total_damage and cand.power > best_target.power:
                best_total_damage = total_damage
                best_target = cand
            if total_damage == best_total_damage and cand.power == best_target.power and cand.initiative > best_target.initiative:
                best_total_damage = total_damage
                best_target = cand
...
while True:
    ...
    print(f"Part 2: {winning_units} units left in winning {winning_team} army - boost {boost}")

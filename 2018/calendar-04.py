...
for shift in shifts:
	...
	for i in range(len(shift["falls_mins"])):
		guards[shift["guard_id"]]["record"][shift["falls_mins"][i]:shift["wakes_mins"][i]] += 1
...
print(f"Part 2: Guard {max_ind_guard_id}: minute {max_ind_min_index} ({max_ind_minute} times)")
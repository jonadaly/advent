...
for i_row, row in enumerate(geologic_index):
    ...
    for i_col, col in enumerate(row):
        if i_col == 0:
            continue
        geologic_index[i_row, i_col] = erosion_level[i_row - 1, i_col] * erosion_level[i_row, i_col - 1]
...
risk_level = np.sum(terrain[:TARGET[1]+1, :TARGET[0]+1] == b"=") + (2*np.sum(terrain[:TARGET[1]+1, :TARGET[0]+1] == b"|"))
...
test = nx.dijkstra_path_length(graph, (0, 0, Gear.TORCH), (TARGET[0], TARGET[1], Gear.TORCH))
print(test)
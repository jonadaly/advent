...
while True:
    ...
    for k, v in valid_values_per_field.items():
        candidate_fields = [
            k2 for k2, v2 in fields.items() if v <= v2 and k2 not in field_map.values()
        ]
...
product_values = np.product(
    [my_ticket[k] for k, v in field_map.items() if v.startswith("departure")]
)

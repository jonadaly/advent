...
def translate(
    location: Tuple[int, int], translation: Tuple[int, int]
) -> Tuple[int, int]:
    return (location[0] + translation[0], location[1] + translation[1])


def rotate(location: Tuple[int, int], angle_deg: int) -> Tuple[int, int]:
    rotated_x = (location[0] * np.cos(angle_deg * math.pi / 180)) - (
        location[1] * np.sin(angle_deg * math.pi / 180)
    )
    rotated_y = (location[1] * np.cos(angle_deg * math.pi / 180)) + (
        location[0] * np.sin(angle_deg * math.pi / 180)
    )
...
for instruction in instructions:
    ...
    value = int(instruction[1:])
    if direc == "F":
        ship_position = translate(
            ship_position, (value * waypoint[0], value * waypoint[1])
        )
    elif direc == "L":
